...
public class ChangeAudioImportSettings : ScriptableObject {
	...
	[MenuItem ("Utils/Sound/Default options to import")]
	static void DefaultOptionsToImport() {
		SetDefaultOptionsToImport();
	}

	[MenuItem ("Utils/Sound/Toggle audio compression/Disable")]
	static void ToggleCompression_Disable() {
		SelectedToggleCompressionSettings(AudioImporterFormat.Native);
	}
	
	[MenuItem ("Utils/Sound/Toggle audio compression/Enable")]
	...
	[MenuItem ("Utils/Sound/Toggle decompress on load/Compressed in memory")]
	...
	[MenuItem("Utils/Sound/Toggle decompress on load/Decompress on load")]
	...
	[MenuItem("Utils/Sound/Toggle decompress on load/Stream from disc")]
	...
	[MenuItem ("Utils/Sound/Toggle 3D sound/Disable")]
	static void Toggle3DSound_Disable() {
		SelectedToggle3DSoundSettings(false);
	}
	
	[MenuItem ("Utils/Sound/Toggle 3D sound/Enable")]
	...
	[MenuItem ("Utils/Sound/Toggle mono/Auto")]
	static void ToggleForceToMono_Auto() {
		SelectedToggleForceToMonoSettings(false);
	}
	
	[MenuItem ("Utils/Sound/Toggle mono/Forced")]
	...
	static void SetDefaultOptionsToImport()
	{
	...
	[MenuItem("Utils/Sound/ChangeNames")]
	static void ChangeAudioNames()
	{
		Object[] audioclips = GetSelectedAudioclips();
		Selection.objects = new Object[0];
		var i = 0;
		foreach (AudioClip audioclip in audioclips) {
			AssetDatabase.RenameAsset(AssetDatabase.GetAssetPath(audioclip),"sniper"+i);
			i++;
		}
	}
}